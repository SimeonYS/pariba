...
class ParibaPipeline:

    # Database setup
    conn = sqlite3.connect('pariba.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `pariba`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM pariba WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO pariba (date, title, link, content)"
